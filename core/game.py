...
# --- DIP: contrato mínimo para cualquier “dado” compatible con el juego ---
...
class BackgammonGame:
    ...
    Mantiene tablero, jugadores, dados y turno.
    """

    def __init__(self, board: Board, white: Player, black: Player, dice: DiceLike):
        """Inicializa la partida sin aplicar reglas todavía."""
        self._board: Board = board
        self._white: Player = white
        self._black: Player = black
        self._dice: DiceLike = dice
        self._current: Player = white   # por ahora empieza blanco
        self._rolled: List[int] = []    # último resultado de dados
    ...
    def get_opponent(self) -> Player:
        ...
        return self._black if self._current == self._white else self._white

    # ------------------ flujo de turnos / dados ------------------

    def end_turn(self) -> None:
        """Pasa el turno al otro jugador y limpia los dados."""
        self._current = self.get_opponent()
        self._rolled = []
    ...
    def roll_dice(self) -> List[int]:
        ...
        self._rolled = list(self._dice.roll())
        return self._rolled
    ...
    def move(self, start: int, end: int, checker_color: str) -> None:
        ...
        if self._has_on_bar(checker_color):
            ...
            dado_usado: Optional[int] = None
            for d in sorted(set(self._rolled), reverse=True):
                if self._entry_point(checker_color, d) == end:
                    dado_usado = d
                    break
            if dado_usado is None:
                raise ValueError(f"Destino inválido para entrar desde barra con estos dados: {self._rolled}")

            # bloqueo/comer en el destino
            dest_count = self._board.count_at(end)
            dest_owner = self._board.owner_at(end)
            ...
            if dest_owner == opponent and dest_count == 1:
                capt = self._board.remove_checker(end)
                self._board.send_to_bar(capt)

            # entrar: saco de la barra y coloco en 'end'
            ficha = self._board.pop_from_bar(checker_color)
            self._board.add_checker(end, ficha)

            # consumir dado y chequear fin de turno
            self._rolled.remove(dado_usado)
            if not self._rolled:
                self.end_turn()
            return
        ...
        if is_bear_off:
            ...
            if need in self._rolled:
                dado_usado = need
            else:
                home = self._home_range(checker_color)
                hay_mas_lejos = any(
                    self._board.owner_at(i) == checker_color and (
                ...
                dado_usado = next((d for d in self._rolled if d > need), None)
                if dado_usado is None:
                    raise ValueError(f"No hay dado exacto {need} ni mayor disponible: {self._rolled}")

            # quitar la ficha del punto (sale del tablero)
            stack = self._board.get_point(start)
            ...
            self._board.remove_checker(start)
        ...
        if dist not in self._rolled:
            raise ValueError(f"El movimiento ({dist}) no coincide con la tirada: {self._rolled}")
        ...
        if dest_owner == opponent and dest_count >= 2:
            raise ValueError(f"Punto bloqueado por {opponent}: {end} tiene {dest_count} fichas.")
        if dest_owner == opponent and dest_count == 1:
            capturada = self._board.remove_checker(end)
            self._board.send_to_bar(capturada)

        # mover
        self._board.move_checker(start, end, target)

        # consumir dado y, si no quedan, pasar turno
        self._rolled.remove(dist)
        if not self._rolled:
            self.end_turn()
    ...
    def _any_on_board(self, color: str) -> bool:
        ...
        for i in range(24):
            if self._board.owner_at(i) == color:
                return True
        return False
    ...
    def legal_moves(self) -> list[tuple[int, int, int]]:
        color = self._current.get_color()
        dice = list(self._rolled)
        if not dice:
            return []
            ...
            for d in set(dice):
                if color == "blanco":
                    ...
                    if 0 <= end <= 23:
                        ...
                        hay_mas_lejos = any(
                            self._board.owner_at(j) == color and j > i
